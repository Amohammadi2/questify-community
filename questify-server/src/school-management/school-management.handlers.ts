...
export class UpdateSchoolHandler implements ICommandHandler<UpdateSchoolCommand> {
  ...
  async execute(command: UpdateSchoolCommand): Promise<SchoolDocument | "not-found"> {
    const res = await this.schoolModel.findOneAndUpdate({ id: command.id }, command.school);
